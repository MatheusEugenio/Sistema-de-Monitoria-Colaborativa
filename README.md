...
* **Integração:** Conexão robusta entre a aplicação e o banco, utilizando boas práticas de manipulação de dados.
* **Funcionalidades:** Implementação de operações de CRUD para o gerenciamento de usuários, disciplinas e histórico de atendimentos.

### O Processo
O desenvolvimento foi realizado em grupo por **José Mayson**, **Moacir Neto** e **Matheus Eugênio**. O maior aprendizado foi compreender como as decisões de design no banco de dados impactam diretamente a consistência e a usabilidade do sistema, sendo uma etapa fundamental para consolidar nosso conhecimento sobre arquitetura de dados na graduação.
